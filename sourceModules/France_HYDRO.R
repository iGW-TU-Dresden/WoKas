...
for(i in 1:nrow(stationInfo)){
  ...
  name <- as.character(stationIds$Name[i])
  ...
  if (length(Data) >=2) {  
    ...
    write.csv2(df, file=con, row.names=FALSE, quote=FALSE)
    close(con)
    
    cat(sprintf("Data exported for station: %s ID: %s\n", name, id)) # Success message
  }
} 
