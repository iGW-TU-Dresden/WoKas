...
# observations from GKD online data portal, Germany
...
for(i in 1:nrow(stationInfo)){  
  ...
  download.file(paste0(dlUrl,'&dl=1'), tmpFileArchive, mode="wb")
  ...
  for( j in 1:nrow(fileInfos)){
    ...
    if (class(dat) != "try-error"){
      ...
      if(coord_skip==0){
        ...
        coord <- read.csv(tmpFileCsv, header=FALSE, sep=';',dec=",", skip=coord_skip-1, nrows=1, col.names=c('long_name','lon','lat_name','lat','projection'), colClasses=c('character','character','character'))
        
        # get coordinate in EPSG 31468 projection
        lon <- as.numeric(gsub('[^0-9.]','',coord$lon)); lat <- as.numeric(gsub('[^0-9.]','',coord$lat)) # extract the coordinates as numeric values from the CSV
        GK <- data.frame(lon_GK=as.numeric(lon),lat_GK=as.numeric(lat)) # paste the coordinates in a new one row data frame
        
        # using sf library (instead rgdal)
        GK <- st_as_sf(GK, coords = c("lon_GK", "lat_GK")) # convert the data frame to an 'sf' object
        st_crs(GK) <- "+proj=tmerc +lat_0=0 +lon_0=12 +k=1 +x_0=4500000 +y_0=0 +ellps=bessel +datum=potsdam +units=m +no_defs" # defining Gauss-Krueger zone 4
        
        # transform coordinates from Gauss-Kruger Zone 4 to WGS84
        WGS84 <- st_transform(GK, crs = "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")
        
        # extracts x (longitude) and y (latitude) coordinates
        lon_GK <- st_coordinates(WGS84)[, 1]  # x (longitude)
        lat_GK <- st_coordinates(WGS84)[, 2]  # y (latitude)
        
        # using rgdal library: not available after year 2023
        # add projection information (actually this converts the data.frame to a variable of class "SpatialPoints")
